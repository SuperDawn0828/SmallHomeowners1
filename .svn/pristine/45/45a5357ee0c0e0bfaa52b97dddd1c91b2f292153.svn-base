...
{
    if (self = [super init]) {
        self.showDuration = 0.25;
        self.hiddenDuration = 0.25;
...
- (void)show
{
    ...
    for (int i = 0; i < imagesCount; i++)
    {
        ...
        {
            // 原本该第三方传入的是NSString数组(本人自定义的是传入url和image混合的)
            if ([self.imagesURL[i] isKindOfClass:[NSString class]]) {
                photo.original_pic = self.imagesURL[i];
                photo.thumbnail_pic = self.imagesURL[i];
            }
            else
            {
                photo.originalImage = self.imagesURL[i];
                photo.thumbnailImage = self.imagesURL[i];
                // 记录图片原始大小（指的是宽为屏幕宽度的大小）
                photo.originalSize = CGSizeMake(PYScreenW, PYScreenW * photo.originalImage.size.height / photo.originalImage.size.width);
            }
        }
