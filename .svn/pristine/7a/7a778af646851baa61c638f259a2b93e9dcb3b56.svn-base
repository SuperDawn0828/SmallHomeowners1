//
//  ZSOrderPersonDetailViewController.m
//  SmallHomeowners
//
...
@interface ZSOrderPersonDetailViewController ()<ZSTextWithPhotosTableViewCellDelegate>
...
@implementation ZSOrderPersonDetailViewController
...
- (void)viewDidLoad
{
    [super viewDidLoad];
    //UI
    [self configureNavgationBar:[NSString stringWithFormat:@"%@信息",[ZSGlobalModel getReleationStateWithCode:global.currentCustomer.releation]] withBackBtn:YES];
    [self configureTable];
    //Data
    [self initData];
}
...
- (void)initData
{
    ...
    //婚姻状况
    if (![self.titleLabel.text containsString:@"配偶信息"]) {
...
- (void)configureTable
{
    [self configureTableView:CGRectMake(0, self.navView.bottom, ZSWIDTH, ZSHEIGHT - self.navView.height) withStyle:UITableViewStyleGrouped];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    else
    {
        ...
        if (self.photoDataArray.count > 0) {
            ZSDynamicDataModel *model = self.photoDataArray[indexPath.row - self.baseDataArray.count];
            cell.model = model;
            //无数据的时候隐藏collectionview
            if (!model.rightData.length) {
                cell.isHiddenCollectionView = YES;
            }
        }
        return cell;
    }
