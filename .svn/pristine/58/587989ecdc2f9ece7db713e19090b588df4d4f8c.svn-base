...
- (void)setupViews
{
    ...
    contentView.backgroundColor = ZSColor(32, 35, 38);
    ...
    label.textColor =ZSColorWhite;// [UIColor colorWithWhite:0.200 alpha:1.000];
    label.font = [UIFont systemFontOfSize:13];
    label.numberOfLines=0;
    [contentView addSubview:label];
    self.messageLabel = label;
    LSGraintCircleLayer *layer = [[LSGraintCircleLayer alloc] initGraintCircleWithBounds:CGRectMake(0, 0, LSProgressHUDCircle,LSProgressHUDCircle)
                                                                                Position:CGPointMake(0, 0)
                                                                               FromColor:[UIColor colorWithWhite:1.000 alpha:0.318]
                                                                                 ToColor:ZSColor(255, 255, 255)
                                                                               LineWidth:4.0];
...
+ (instancetype)showWithMessage:(NSString *)message
{
    ...
    return [self createViewToView:window message:message];
}
...
- (instancetype)setViewsMessageAndFrame:(NSString *)message toView:(UIView *)view
{
    ...
    if (message.length > 0) {
    labelHeight = [message boundingRectWithSize:CGSizeMake(labelWidth, CGFLOAT_MAX)
