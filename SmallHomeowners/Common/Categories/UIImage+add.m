...
+(UIImage *)getImageByType:(NSString*)name {
    ...
    else if ([name isEqualToString:@"婚姻状况"]) {
        image = ImageName(@"bar_marital_s");
    }
    else if ([name isEqualToString:@"银行流水"]) {
        image = ImageName(@"bar_bankwater_s");
    }
    else if ([name isEqualToString:@"收入证明"]) {
        image = ImageName(@"bar_income_proof_s");
    }
    else if ([name isEqualToString:@"财力证明"]) {
        image = ImageName(@"bar_finanacial_s");
    }
    else if ([name isEqualToString:@"不动产权证"]) {
        image = ImageName(@"bar_property_rights_s");
    }
    else if ([name isEqualToString:@"公证书"]) {
        image = ImageName(@"bar_notarization_s");
    }
    else if ([name isEqualToString:@"审批单"]) {
        image = ImageName(@"bar_approval_for_s");
    }
    else if ([name isEqualToString:@"放款凭证"]) {
        image = ImageName(@"bar_advances_vouchers_s");
    }
    else if ([name isEqualToString:@"他项权证"]) {
        image = ImageName(@"bar_hexiang_s");
    }
    else if ([name isEqualToString:@"产权情况表"]) {
        image = ImageName(@"bar_property_report_s");
    }
    else if ([name isEqualToString:@"预评估报告"]) {
        image = ImageName(@"bar_appraisal_report_s");
    }
    else if ([name isEqualToString:@"收款凭证"]) {
    ...
    else if ([name isEqualToString:@"买方还款银行卡"]) {
        image = ImageName(@"list_maifang_buy");
    }
    else if ([name isEqualToString:@"卖方收款银行卡"]) {
        image = ImageName(@"list_maifang_send");
    }
    else { //其他随意加的资料默认同一个logo
