...
- (void)createBottomBtn
{
    ...
    [self.bottomBtn setBackgroundImage:[ZSTool createImageWithColor:ZSColorRed] forState:UIControlStateHighlighted];
...
- (NSInteger)collectionView:(UICollectionView *)collectionView numberOfItemsInSection:(NSInteger)section
{
    ...
    switch (self.addDataStyle) {
        case ZSAddResourceDataOne:
            return 1;
            break;
        case ZSAddResourceDataTwo:
            return 2;
            break;
        case ZSAddResourceDataCountless:
...
//            }
    ...
    if (!self.isShowAdd) {//没有添加功能。同时没有删除功能
        cell.deleteBtn.hidden = YES;
    }
    return cell;
}
...
{
    if (self.selectEnable && [self.titleNameArray count]) {//当前有可操作权限的时候且资料获取成功才可点击，否则点击会闪退
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info
{
    ...
    [USER_DEFALT setInteger:needUploadNum forKey:KneedUploadNum];
    if (self.delegate && [self.delegate respondsToSelector:@selector(showProgress:)]){
        [self.delegate showProgress:[NSString stringWithFormat:@"已上传%ld/%ld",(long)hasbeenUploadNum,(long)needUploadNum]];
    }
    //1.银行卡信息
...
- (void)GoToPhotoLibraryWithDataStyle:(ZSAddResourceDataStyle)dataStyle
{
    ...
    [imagePickerVc setDidFinishPickingPhotosHandle:^(NSArray<UIImage *> *photos, NSArray *assets, BOOL isSelectOriginalPhoto) {
        ...
        [USER_DEFALT setInteger:needUploadNum forKey:KneedUploadNum];
        if (weakSelf.delegate && [weakSelf.delegate respondsToSelector:@selector(showProgress:)]){
            [weakSelf.delegate showProgress:[NSString stringWithFormat:@"已上传%ld/%ld",(long)hasbeenUploadNum,(long)needUploadNum]];
        }
        //异步上传
...
#pragma mark 原图
                [assets enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
                    ...
                    [[TZImageManager manager]getOriginalPhotoWithAsset:imageAssets newCompletion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
                        //图片文件转成data
                        UIImage *imagerotate = [UIImage fixOrientation:photo];//修正图片方向
...
- (void)getProductSaleLoanByUploadFileModel:(ZSWSFileCollectionModel *)fileModel data:(NSData *)data isFromBanker:(BOOL)isFromBanker
{
    ...
    RecursiveMethod = ^(NSInteger value){
        ...
        [ZSRequestManager uploadImageWithNativeAPI:data SuccessBlock:^(NSDictionary *dic) {
            ...
            [USER_DEFALT setInteger:hasbeenUploadNum forKey:KhasbeenUploadNum];
            if (weakSelf.delegate && [weakSelf.delegate respondsToSelector:@selector(showProgress:)]){
                [weakSelf.delegate showProgress:@""];
            }
//            //3.如果是银行卡上传,识别一下
//            if (isFromBanker == YES) {
...
//            }
            ...
            if(value == 0){
                [ZSTool showMessage:@"上传失败,请点击按钮重试" withDuration:DefaultDuration];
                //图片上传失败,传递标志,用于点击保存按钮的时候发送通知,重新上传照片
                if (weakSelf.delegate && [weakSelf.delegate respondsToSelector:@selector(judePictureUploadingFailure:)]){
                    [weakSelf.delegate judePictureUploadingFailure:YES];
                }
            }
            //失败后也要解锁
            [lock unlock];
...
//        if (weakSelf.delegate && [weakSelf.delegate respondsToSelector:@selector(getBankerInfoWithBankerModel:)]){
//            [weakSelf.delegate getBankerInfoWithBankerModel:bankModel];
//        }
//        //2.代理方法，刷新数据
//        [weakSelf reloadDataForColltionviewDelegate];
//    } ErrorBlock:^(NSError *error) {
...
- (void)setModelAttributeWithFileModel:(ZSWSFileCollectionModel *)fileModel  Model:(ZSWSFileCollectionModel *)selectModel isFormAbum:(BOOL)isFormAbum
{
    ...
    fileModel.subCategory  = selectModel.subCategory; //新房见证用于区分(户主页和个人页)星速贷区分是本人还是配偶
...
{
    //刷新高度坐标
    if (self.delegate && [self.delegate respondsToSelector:@selector(refershDataCollectionViewHegiht)]){
        [self.delegate refershDataCollectionViewHegiht];
    }
    //点击大图
    if (self.delegate &&[self.delegate respondsToSelector:@selector(didSelectedImage)]) {
        [self.delegate didSelectedImage];
    }
    //刷新数据
    [self.myCollectionView reloadSections:[NSIndexSet indexSetWithIndex:self.currentIndexPath.section]];
}
...
- (CGFloat)resetCollectionViewFrame
{
    ...
    if ([self.itemArray count]) {
        ...
        //两个加号的时候高度
        if (self.addDataStyle == ZSAddResourceDataTwo){
            hight += self.itemArray.count * (itemHeight + 42) + 30 *self.itemArray.count ;
        }
        if (self.addDataStyle == ZSAddResourceDataCountless){
            //可添加多张图片的高度
...
- (void)deleteDataByIndexPath:(NSIndexPath*)indexPath
{
    ...
    //更新cell高度
    if (self && [self.delegate respondsToSelector:@selector(refershDataCollectionViewHegiht)]){
        [self.delegate refershDataCollectionViewHegiht];
    }
    //有删除操作,代理传值,提示上传张数
    if (self.delegate && [self.delegate respondsToSelector:@selector(showProgress:)]){
